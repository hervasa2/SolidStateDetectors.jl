...
ScalarPotential(::PointTypes{T, N, S, AT, D}, data, grid) where {T, N, S, AT, D} = PointTypes(data, grid, D)
    ...
    for i1 in eachindex(grid[1])
        for i2 in eachindex(grid[2])
            for i3 in eachindex(grid[3])
                data[i1, i2, i3] = get_interpolation(ep_itp, getpoint(grid, i1, i2, i3), S)
            end
        end
    end
