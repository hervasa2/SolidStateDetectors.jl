...
function _get_decomposed_surfaces_torus(t::Torus{T}) where {T}
...
function sample(t::Torus{T}, step::Real) where {T}
    ...
    φMin::T, φMax::T, _ = get_φ_limits(t)
    samples = [
        CylindricalPoint{T}(t.r_torus+r_tube*cos(θ),φ,r_tube*sin(θ)+t.z)
...
function sample(t::Torus{T}, Nsamps::NTuple{3,Int}) where {T}
    ...
    samples = [
        CylindricalPoint{T}(t.r_torus+r_tube*cos(θ),φ,r_tube*sin(θ)+t.z)
        for r_tube in (Nsamps[1] ≤ 1 ? 0 : range(r_tubeMin, r_tubeMax, length = Nsamps[1]))
